...
run_model <- function(simdata, species_list=unique(data$species), n_chains=4, n_iter=10000) {
  ...
  for (i in species_list) {
    ...
    simdata1 <- reshape2::dcast(data = simdata1,formula = site~year,fun.aggregate = function(x) round(mean(x))
                                  ,value.var = "obs")
    rownames(simdata1) <- simdata1$site
    simdata1 <- simdata1[,-1]
    simdata1 <- t(apply(simdata1, 1, na.locf))
...
# increased prob of detection of focal species
create_data1 <- function(nspecies=20, nsite=100, nyear=10, decline=FALSE) {
  # one focal species - probability of detection increases over time
  ...
  for (j in 1: nsite) {
    for (t in 1:nyear) {
      # fixed probability of detection
      p[j,t] <- 0.4+(t-1)/90
      simfocal[j,t] <- rbinom(1, y[j,t], p[j,t])
    }
  }
  ...
  for (i in 1:nspecies) {
    ...
    for (j in 1: nsite) {
      for (t in 1:nyear) {
        # beta distribution from Isaac et al.
        p[j,t] <- rbeta(1, 2, 2)
        sims2[j,t] <- rbinom(1, y[j,t], p[j,t])
      }
    }
